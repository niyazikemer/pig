---
name: exercise-time
description: Triggers when the user is rewriting Zig code they already have, or says they want to do an exercise. A common signal is the user creating a copy of a source file with a `_ref` suffix and sharing it as reference. Also triggers when the user explicitly asks for help writing what comes next in such an exercise.
---

# Exercise time

The user is rewriting or redesigning Zig code from memory and wants to be led
through it. They may hand over reference code (the original Zig source). Treat
