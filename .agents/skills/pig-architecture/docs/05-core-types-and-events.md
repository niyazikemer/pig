...
Phase 1 defines the provider-neutral objects that later pig phases share.

## Why these types exist

The provider layer, agent loop, harness, tools, sessions, and UI should not exchange provider-specific objects.
Instead, they use pig's own message, tool, result, and event models.

## Messages

Messages live in `pig_agent.messages`:
...
Events live in `pig_agent.events`. They describe progress from the portable agent layer:
...
OpenAI, or other API payloads into pig types in a later phase.
